...
public class SuaCongTrinh extends JFrame implements ActionListener{
	...
	public void actionPerformed(ActionEvent e) {
		...
		//Sua
			...
			if(validData()) {
				CongTrinh ct = reverseCTFromTxtField();
				if(daoCT.suaCongTrinh(ct)) {
	...
	public void getCongTrinhTheoMa(String ma) throws ParseException {
		ArrayList<CongTrinh> dsCT = daoCT.layNLDTheoMa(ma);
		
		for (CongTrinh ct : dsCT) {
			txtMaCT.setText(ma);
			txtTenCT.setText(ct.getTenCT());
			txtDiaChi.setText(ct.getDiaDiemCT());
			Date s = ct.getNgayCapPhep();
			Date s1 = ct.getNgayKhoiCong();
			Date s2 = ct.getNgayHoanThanhDuKien();
			...
			//java.util.Date ngayCP = new SimpleDateFormat("yyyy-MM-dd").parse(s);
			//java.util.Date ngayKC = new SimpleDateFormat("yyyy-MM-dd").parse(s1);
			//java.util.Date ngayHT = new SimpleDateFormat("yyyy-MM-dd").parse(s2);
			date_CapPhep.setDate(s);
			date_KhoiCong.setDate(s1);
			date_HoanThanh.setDate(s2);
			
		}
		
	}
	public CongTrinh reverseCTFromTxtField(){
	...
	private boolean validData() {
		...
		// Có 6 ký tự 3 chữ, theo sau là 3 ký số
